...
function runUntilBlocking(){
  while(true){
    ...
    if (l.t === 'BG'){
      // 배경이 실제로 바뀌면(장소 전환) 이전 배경에 남아있던 캐릭터 배지를 정리한다.
      // 대본에 HIDE가 누락된 경우가 있어(예: 아비게일) 장소 전환을 신뢰 가능한 정리 시점으로 삼는다.
      if (l.v !== state.currentBg){ state.visibleChars.clear(); renderChars(); }
      state.currentBg = l.v;
