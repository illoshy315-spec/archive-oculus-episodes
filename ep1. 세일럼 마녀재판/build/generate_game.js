...
// 문장 단위 텍스트 분할 — 긴 NARR/THINK/대사 한 줄을 텍스트박스 분량(약 2~3줄)씩 순차 출력한다.
// 원본 대본은 건드리지 않고, 렌더링 시점에만 나눈다.
function splitTextChunks(text){
  ...
  for (const raw of sentences){
    const s = raw.trim();
    if (!s) continue;
    if (cur && (cur.length + s.length + 1) > budget){ chunks.push(cur); cur = s; }
...
function runUntilBlocking(){
  while(true){
    ...
    if (l.t === 'TEXT'){
      const chunks = splitTextChunks(l.text);
      state.textQueue = chunks.slice(1);
      renderText(l, chunks[0]);
      return;
    }
...
function renderText(l, chunkText){
  ...
  textEl.textContent = (chunkText !== undefined) ? chunkText : l.text;
  const hasMore = state.textQueue && state.textQueue.length > 0;
  $('#advanceHint').textContent = hasMore ? '계속 ▶' : '클릭 / Space ▶';
...
function advance(){
  ...
  if (state.textQueue && state.textQueue.length > 0){
    ...
    const l = scene.lines[state.lineIdx];
    renderText(l, state.textQueue.shift());
    return;
  }
  const scene = currentScene();
  const l = scene.lines[state.lineIdx];
