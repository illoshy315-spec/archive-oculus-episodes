...
`references/steps/step3.md` + `references/npc_card_template.md` + `references/choice_types.md` 로드.
**순서 엄수**: 포맷 결정 → 파트 구조 확정 → 파트별 지뢰 설계(파트당 2~3개 후보) → 파트당 1개 선택 → 신분 역산 → 고정NPC 등장 여부 → NPC 카드 → scenario.json 골격.
